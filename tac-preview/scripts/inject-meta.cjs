  ...
  const heroBoot = item.path === '/'
    ? `\n    <div id="hero-boot" aria-hidden="true" style="position:fixed;inset:0;z-index:40;overflow:hidden;background:#1B1A18;color:#fff">
      ...
      <div class="relative z-10 min-h-screen min-h-[100svh] flex flex-col justify-center md:justify-end pt-24 pb-10 md:pt-28 md:pb-16 px-6 md:px-14 lg:px-20 max-w-[1500px] mx-auto">
        ...
        <div class="font-display font-bold text-[34px] sm:text-[50px] md:text-[68px] xl:text-[92px] leading-[1.0] md:leading-[0.98] tracking-[-0.04em] text-white max-w-[1100px] mb-4 md:mb-6" style="text-shadow:1px 1px 12px rgba(0,0,0,0.15)">Age should<br/>never define you.</div>
    ...
    <script>(function(){var b=document.getElementById('hero-boot');if(!b)return;var ua=navigator.userAgent||'';if(navigator.webdriver===true||/bot|crawl|spider|slurp|headless|lighthouse|screaming|gptbot|oai-searchbot|chatgpt|claudebot|anthropic|perplexity|bingpreview|google-inspectiontool|googleother|applebot|bytespider|amazonbot|google\\s*web\\s*preview/i.test(ua)){return;}var r=document.getElementById('root'),g=false;function rm(){if(g)return;g=true;b.style.transition='opacity .35s ease';b.style.opacity='0';setTimeout(function(){b.parentNode&&b.parentNode.removeChild(b);},400);}function c(){if(r&&r.children.length>0){requestAnimationFrame(function(){requestAnimationFrame(rm);});}else{requestAnimationFrame(c);}}requestAnimationFrame(c);setTimeout(rm,2500);['touchstart','pointerdown','keydown','wheel','touchmove','scroll'].forEach(function(e){window.addEventListener(e,rm,{once:true,passive:true,capture:true});});})();</script>`
    : ''
